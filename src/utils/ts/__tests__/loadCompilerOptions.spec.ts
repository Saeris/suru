...
describe(`loadCompilerOptions`, () => {
  it(`loads the compiler options for the given project`, async () => {
  ...
  it(`throws an error if the file doesn't exist`, () => {
    expect(async () => loadCompilerOptions(`foo`)).toThrow(
      /Unable to find a configuration file in/
    );
  });

  it(`throws an error when given an invalid tsconfig`, async () => {
    const ts = await import(`typescript`);
    jest.spyOn(ts.sys, `fileExists`).mockReturnValueOnce(true);
    expect(async () => loadCompilerOptions(`foo`)).toThrow(/Error reading typescript configration/);
    jest.restoreAllMocks();
  });
});
